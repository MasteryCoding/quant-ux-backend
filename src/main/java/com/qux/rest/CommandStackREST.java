  ...
  public void setPos(RoutingContext event, int dif) {
    ...
    this.acl.canWrite(getUser(event), event, allowed -> {
      if (allowed) {
        ...
        logger.error("CommandStackRest.setPos() > User " + getUser(event) + " tried to setPos without permission");
  ...
  public void setPos(RoutingContext event, int dif, String appID) {
    ...
    mongo.findOne(table, AppPart.findByApp(appID), null, res -> {
      ...
      if (res.succeeded()) {
        ...
        logger.error("CommandStackRest.setPos() > User " + getUser(event) + " tried to setPos on not existing app");
        returnError(event, 404);
      }
    });
  }
    ...
    try {
      ...
      this.acl.canWrite(getUser(event), event, allowed -> {
        if (allowed) {
          ...
          logger.error("CommandStackRest.pop() > User " + getUser(event) + " tried to pop ");
          returnError(event, 401);
        }
      });
  ...
  public void pop(final RoutingContext event, final int count, final String appID) {
    ...
    mongo.findOne(table, AppPart.findByApp(appID), null, res -> {
      ...
      if (res.succeeded()) {
        ...
        logger.error("CommandStackRest.pop() > User " + getUser(event) + " tried to pop not exiting app ");
        returnError(event, 404);
      }
    });
  }
      ...
      if (errors.isEmpty()) {
        this.acl.canWrite(getUser(event), event, allowed -> {
          if (allowed) {
            ...
            logger.error("CommandStackRest.add() > User " + getUser(event) + " tried to add command without write!"
                + command.encodePrettily());
            returnError(event, 401);
          }
        });
      } else {
        logger.error("CommandStackRest.add() > User " + getUser(event) + " tried to add shitty command > ");
      ...
      if (res.succeeded()) {
        ...
        mongo.update(table, Model.findById(id), update, res2 -> {
          if (res2.succeeded()) {
            ...
            logger.error("CommandStackRest.add() > Mongo error " + res2.cause() + " >> " + update.encodePrettily());
          }

        });

      } else {
        logger.error("CommandStackRest.add() > User " + getUser(event) + " tried to add command to not exsiting app");
        returnError(event, 404);
      }
    });
  }
