...
public class AppREST extends MongoREST {
  ...
  public void delete(RoutingContext event, String appID) {
    ...
    /**
     * First we set a "isDeleted flag" and return the request, Then we delete the
     * document
     */
      ...
      if (allowed) {
        ...
        if (json.startsWith("[") && json.endsWith("]")) {
          try {
            ...
            logger.error("AppREST.applyChanges() > User " + getUser(event) + " >> Could not parse json: " + json);
            returnError(event, 405);
          }
        } else {
        ...
        logger.error("AppREST.applyChanges() > User " + getUser(event) + " tried to write to app " + appID);
  ...
  public void applyChanges(String appID, JsonArray changes, RoutingContext event) {
    ...
    mongo.updateCollection(table, Model.findById(appID), update, res -> {
      if (res.succeeded()) {
        ...
        logger.error("AppREST.applyChanges() > User " + getUser(event) + " >> Mongo Error " + res.cause().getMessage()
  ...
  private void findEmbedded(RoutingContext event) {
    ...
    this.acl.canRead(getUser(event), event, allowed -> {
      if (allowed) {
        ...
        logger.error("AppREST.findEmbedded() > User " + getUser(event) + " tried to read without permission");
        returnError(event, 401);
      }
    });
