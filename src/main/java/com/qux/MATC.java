...
public class MATC extends AbstractVerticle {
  ...
  private void initCommentRest(Router router) {
    ...
    router.route(HttpMethod.GET, "/rest/comments/hash/:hash/:appID/:reference/:type.json")
        .handler(invitationComment.findBy());

    router.route(HttpMethod.GET, "/rest/comments/count/hash/:hash/:appID/:type.json")
        .handler(invitationComment.count());
    router.route(HttpMethod.GET, "/rest/comments/count/hash/:hash/:appID/:reference/:type.json")
        .handler(invitationComment.count());

    router.route(HttpMethod.GET, "/rest/comments/hash/:hash/:appID/:type/count.json")
        .handler(invitationComment.count());
    router.route(HttpMethod.GET, "/rest/comments/hash/:hash/:appID/:reference/:type/count.json")
        .handler(invitationComment.count());

    router.route(HttpMethod.POST, "/rest/comments/hash/:hash/:appID").handler(invitationComment.create());
    router.route(HttpMethod.POST, "/rest/comments/hash/:hash/:appID/:commentID.json")
