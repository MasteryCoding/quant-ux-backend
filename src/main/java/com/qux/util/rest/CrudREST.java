      ...
      this.acl.canWrite(getUser(event), event, allowed -> {
        if (allowed) {
          ...
          this.getACLList(user, userApps -> {
            ...
            logger.error(
                "CrudRest.update() > User " + getUser(event) + " tried to UPDATE without permission. ACL: " + userApps);
          });
      ...
      this.acl.canRead(getUser(event), event, allowed -> {
        if (allowed) {
          ...
          this.getACLList(user, userApps -> {
            ...
            logger.error(
                "CrudRest.find() > User " + getUser(event) + " tried to read without permission. ACL: " + userApps);
          });
          returnError(event, 401);
        }
      });
      ...
      this.acl.canRead(getUser(event), event, allowed -> {
        if (allowed) {
          ...
          this.getACLList(user, userApps -> {
            ...
            logger.error("CrudRest.findByAcl() > User " + getUser(event) + " tried to read without permission. ACL: "
                + userApps);
          });
          returnError(event, 401);
        }
      ...
      this.acl.canRead(getUser(event), event, allowed -> {
        if (allowed) {
          ...
          this.getACLList(user, userApps -> {
            ...
            logger.error("CrudRest.countByAcl() > User " + getUser(event) + " tried to read without permission. ACL: "
                + userApps);
          });
